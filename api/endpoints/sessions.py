...
def sessions_summary():
    ...
    return jsonify({
        ...
        "avg_time_on_page": float(row.avg_time_on_page) if row.avg_time_on_page is not None else None,
        "overall_bounce_rate": (
            float(bounce_row.overall_bounce_rate) if bounce_row.overall_bounce_rate is not None else None
        ),
    })

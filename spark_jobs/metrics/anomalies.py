...
def _iqr_flags(df: DataFrame, metrics: list[str]) -> DataFrame:
    ...
    bounds = df.groupBy("dt").agg(
        *[F.percentile_approx(F.col(m), [0.25, 0.75]).alias(f"_q_{m}") for m in metrics]
    )
    for m in metrics:
        bounds = bounds.withColumn(f"_q1_{m}", F.col(f"_q_{m}").getItem(0))
        bounds = bounds.withColumn(f"_q3_{m}", F.col(f"_q_{m}").getItem(1))
        bounds = bounds.withColumn(f"_iqr_{m}", F.col(f"_q3_{m}") - F.col(f"_q1_{m}"))
        bounds = bounds.withColumn(f"_lo_{m}", F.col(f"_q1_{m}") - 1.5 * F.col(f"_iqr_{m}"))
        bounds = bounds.withColumn(f"_hi_{m}", F.col(f"_q3_{m}") + 1.5 * F.col(f"_iqr_{m}"))
    ...
    for m in metrics:
        out = out.withColumn(
            f"flag_iqr_{m}",
            ((F.col(m) < F.col(f"_lo_{m}")) | (F.col(m) > F.col(f"_hi_{m}"))).cast("int"),
        )
...
def _anomaly_type(df: DataFrame) -> DataFrame:
    ...
    return df.withColumn(
        "anomaly_type",
        F.when(
            F.col("flag_z_events_per_minute") + F.col("flag_iqr_events_per_minute") > 0,
            F.lit("high_velocity"),
        )
        .when(
            F.col("flag_z_session_duration_seconds") + F.col("flag_iqr_session_duration_seconds")
            > 0,
            F.lit("extreme_duration"),
        )
        .when(
            F.col("flag_z_total_events") + F.col("flag_iqr_total_events") > 0, F.lit("high_volume")
        )
        .when(
            F.col("flag_z_unique_pages") + F.col("flag_iqr_unique_pages") > 0,
...
def compute(events: DataFrame) -> DataFrame:
    ...
    flagged = flagged.withColumn("total_flags", sum(F.col(c) for c in flag_cols))

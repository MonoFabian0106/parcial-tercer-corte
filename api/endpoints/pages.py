...
def top_pages():
    try:
        ...
        limit = parse_int(request.args.get("limit"), field="limit", default=10, minimum=1, maximum=100)
    ...
    if metric == _METRIC_TIME:
        ...
        row_to_dict = lambda r: {  # noqa: E731
            ...
            "views": r.views,
            "avg_time_on_page": float(r.avg_time_on_page) if r.avg_time_on_page is not None else None,
            "p95_time_on_page": float(r.p95_time_on_page) if r.p95_time_on_page is not None else None,
        }
    else:
        query = text(
            """
